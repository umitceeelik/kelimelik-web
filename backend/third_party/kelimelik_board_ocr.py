...
# - Cihaz JSON'undan board üst boşluğu (top_frac) seçimi
# - 15x15 hücreleri out/cells/ altına yazma
# - Taş içerenleri out/cells_occupied/ altına da kopyalama
# - Rack bandını bulup out/rack/ altına parça parça yazma
# - --export-only sadece uyumluluk için (yoksayılır)
# -----------------------------------------------------------------------------
...
import argparse
import numpy as np
from typing import Optional   # ✅ eklendi

# JSON bulunamazsa/uygun eşleşme olmazsa: S23 Ultra örneği
DEFAULT_TOP_FRAC = 993.0 / 3088.0
...
# ---------- Cihaz JSON'u: ekran oranına en yakın top_frac seç ----------
def _load_devices_json(path: str):
    """devices.json -> [{name,w,h,top_frac,ar}] listesi (h/w = ar)."""
    try:
...
def _pick_top_frac_for_image(H: int, W: int, devices, ar_tol: float = 0.03):
    """En-boy oranı en yakın cihazdan top_frac seçimi."""
    if not devices:
        return None, None
    ar_img = float(H) / float(W)
    scored = [(abs(d["ar"] - ar_img), abs(d["w"] - W), d) for d in devices]
...
# ---------- Tahta ROI: sabit oranla kırp ----------
def board_box_fixed_ratio(
    full_bgr,
    top_frac: float,
    debug_dir: Optional[str] = None,   # ✅ düzeltildi
    tag: str = "fixed"
):
    ...
    side = W
    B = T + side
    if B > H:  # taşma varsa yukarı kaydır
        shift = B - H
        T = max(0, T - shift)
        B = H
    L, R = 0, W
...
def white_text_ratio(bgr) -> float:
    hsv = cv2.cvtColor(bgr, cv2.COLOR_BGR2HSV)
    S = hsv[..., 1]; V = hsv[..., 2]
...
def bonus_color_ratio(bgr) -> float:
    hsv = cv2.cvtColor(bgr, cv2.COLOR_BGR2HSV)
...
def is_real_tile(bgr) -> bool:
    if tile_bg_ratio(bgr) < 0.11:
        return False
    a, ext, hf = letter_stats(bgr)
    if not (a >= 0.0045 and ext >= 0.11 and hf >= 0.24):
        return False
    if bonus_color_ratio(bgr) > 0.06 and a < 0.012:
        return False
    if white_text_ratio(bgr) > 0.035 and a < 0.012:
        return False
    return True


# ---------- Rack bandını bul ----------
def auto_find_rack_band(full, L: int, T: int, R: int, B: int):
    h, w = full.shape[:2]
    hsv = cv2.cvtColor(full, cv2.COLOR_BGR2HSV)
    mask = cv2.inRange(hsv, np.array([10, 40, 80]), np.array([45, 255, 255]))
    col = (mask / 255).mean(axis=1)
    col[:B] = 0
...
def main():
    ...
    if full is None:
        raise SystemExit("Görsel açılamadı")
    H, W = full.shape[:2]

    top_frac, picked = (None, None)
    ...
    if rt is not None and rb is not None and rb - rt > 10:
        strip = full[rt:rb, :]
        if dbg_dir:
            cv2.imwrite(os.path.join(dbg_dir, "rack_strip.png"), strip)
        ...
        for i, v in enumerate(on):
            if v and s is None:
                ...
                ranges.append((s, i - 1)); s = None
        if s is not None:
            ranges.append((s, len(on) - 1))
        if len(ranges) > 7:
